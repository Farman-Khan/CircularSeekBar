package com.library.practice.circularseekbar;
...
/**
 * Created by fk on 1/3/2017.
 */

public class CSBar extends View {
    ...
    private OnCSBarChangeListener mOnCSBarChangeListener;

    public CSBar(Context context) {
        super(context);
        init(context, null);
    }

    public CSBar(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            final TypedArray a = context.obtainStyledAttributes(attrs,
                    R.styleable.CSBar, 0, 0);

            Drawable indicatorIcon = a.getDrawable(R.styleable.CSBar_indicatorIcon);
            ...
            mPoints = a.getInteger(R.styleable.CSBar_points, mPoints);
            mMin = a.getInteger(R.styleable.CSBar_min, mMin);
            mMax = a.getInteger(R.styleable.CSBar_max, mMax);
            mStep = a.getInteger(R.styleable.CSBar_step, mStep);

            mProgressWidth = (int) a.getDimension(R.styleable.CSBar_progressWidth, mProgressWidth);
            progressColor = a.getColor(R.styleable.CSBar_progressColor, progressColor);

            mArcWidth = (int) a.getDimension(R.styleable.CSBar_arcWidth, mArcWidth);
            arcColor = a.getColor(R.styleable.CSBar_arcColor, arcColor);

            mTextSize = (int) a.getDimension(R.styleable.CSBar_textSize, mTextSize);
            textColor = a.getColor(R.styleable.CSBar_textColor, textColor);

            mClockwise = a.getBoolean(R.styleable.CSBar_clockwise,
                    mClockwise);
            mEnabled = a.getBoolean(R.styleable.CSBar_enabled, mEnabled);
            a.recycle();
        }
        ...
        if (mEnabled) {
            ...
            switch (event.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    if (mOnCSBarChangeListener != null)
                        mOnCSBarChangeListener.onStartTrackingTouch(this);
                    updateOnTouch(event);
                    break;
                ...
                case MotionEvent.ACTION_UP:
                    if (mOnCSBarChangeListener != null)
                        mOnCSBarChangeListener.onStopTrackingTouch(this);
                    setPressed(false);
...
//			System.out.printf("%d, %f\n", progress, mProgressSweep);
    ...
    public interface OnCSBarChangeListener {

        /**
         * Notification that the point value has changed.
         *
         * @param CSBar The CSBar view whose value has changed
         * @param points     The current point value.
         * @param fromUser   True if the point change was triggered by the user.
         */
        void onPointsChanged(CSBar CSBar, int points, boolean fromUser);

        void onStartTrackingTouch(CSBar CSBar);

        void onStopTrackingTouch(CSBar CSBar);
    }
    ...
    public void setOnCSBarChangeListener(OnCSBarChangeListener onCSBarChangeListener) {
        mOnCSBarChangeListener = onCSBarChangeListener;
    }
}
